...
Before installing External Secrets, you need to prepare the following components since all secrets are stored in Alibaba Cloud KMS:

### 1. Alibaba Cloud KMS Setup
- **Create KMS Instance**: Ensure you have an active Alibaba Cloud KMS service in your region
- **Create Secrets**: Store your application secrets in Alibaba Cloud KMS using the following naming convention:
  ```
  /your-project/env/service/secret-name
  Example: /zyql/prod/database/credentials
  ```

### 2. Authentication Configuration

#### Option A: RRSA Authentication (Recommended for ACK)
If you're using Alibaba Cloud Container Service for Kubernetes (ACK):
- **Configure RRSA**: Set up Role-based Service Account (RRSA) for your cluster
- **Create IAM Role**: Create an IAM role with KMS access permissions
- **Set Values**: Configure `cloud: "alicloud"` in values.yaml

Required permissions for the IAM role:
```json
{
  "Version": "1",
  "Statement": [
...
For non-ACK clusters or when RRSA is not available:
- **Create Access Keys**: Generate AccessKey ID and AccessKey Secret with KMS permissions in Alibaba Cloud RAM
- **Create Kubernetes Secret**: Manually create a secret in the `external-secrets` namespace:
  ```bash
  kubectl create secret generic volcengine-access-kms-secret \
    --from-literal=access-key-id=<your-access-key-id> \
    --from-literal=access-key-secret=<your-access-key-secret> \
    -n external-secrets
  ```
- **Set Values**: Configure `cloud: "volcengine"` in values.yaml
...
Due to network restrictions, open source images may pull very slowly. Before deployment, you need to upload the open source images to `reg.zyql.com` and create a Docker registry secret in your Kubernetes cluster.

#### Step 1: Upload Images to Private Registry
Upload the following External Secrets images to your private registry `reg.zyql.com`:

```bash
# Original images from public registry
docker pull ghcr.io/external-secrets/external-secrets:v0.19.2

# Tag for private registry
docker tag ghcr.io/external-secrets/external-secrets:v0.19.2 reg.zyql.com/library/external-secrets:v0.19.2

# Push to private registry
docker push reg.zyql.com/library/external-secrets:v0.19.2
```

#### Step 2: Create Docker Registry Secret
Create a Docker registry secret in the `external-secrets` namespace:

```bash
kubectl create secret docker-registry regcred \
  --docker-server=reg.zyql.com \
  --docker-username=<your-username> \
  --docker-password=<your-password> \
  --docker-email=<your-email> \
  -n external-secrets
```
However, the "reg.zyql.com/library" is a public repository, so you can skip this step if you don't need authentication.
...
### Initial Deployment Issues with ArgoCD

When deploying External Secrets via ArgoCD, you may encounter the following issues after automatic sync:

#### 1. Annotation Size Limit Error
If you see this error in the Live Manifest:
```
metadata.annotations: Too long: may not be more than 262144 bytes
```

#### 2. CustomResourceDefinition Not Found
If you encounter errors like:
```
Resource not found in cluster: apiextensions.k8s.io/v1/CustomResourceDefinition:clustersecretstores.external-secrets.io
```

#### 3. Other Resource Not Found Errors
Similar "resource not found in cluster" errors for other External Secrets resources.

### Solution

For these issues, use ArgoCD's **Desired Manifest** to perform `kubectl create` operations instead of `kubectl apply`:

1. Navigate to your ArgoCD application
2. Go to the **Desired Manifest** tab
3. Copy the manifest content for the failing resources
4. Apply using `kubectl create` instead of `kubectl apply`:
   ```bash
...
- **secretStoreRef**: Reference to the ClusterSecretStore that contains connection details to your external secret provider
- **target**: The Kubernetes Secret that will be created/updated
- **data**: Array of secret keys to sync, with their corresponding remote references

The ExternalSecret will create a Kubernetes Secret named `example-secret` with the specified keys populated from the external provider.
